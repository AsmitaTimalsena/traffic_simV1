...
try:
    ...
    while running:
        ...
        # North road dashed separators (increased to end at the same position as the solid line)
        left_north_dash = north_south_center - 2 * lane_width
        right_north_dash = north_south_center + 2 * lane_width

        # Extend the dashed lines to ensure they end in alignment with the intersection boundary
        draw_dashed_line(screen, WHITE, (left_north_dash, 0), (left_north_dash, intersection_top + dash_length))
        draw_dashed_line(screen, WHITE, (right_north_dash, 0), (right_north_dash, intersection_top + dash_length))
